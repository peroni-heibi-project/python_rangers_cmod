...
class BibliographicEntityQueryHandler(QueryHandler):
    ...
    def getById(self, id) -> pd.DataFrame:
        with connect(self.dbPathOrUrl) as con:
            query = """
                SELECT DISTINCT be.internalId, be.title, be.pub_date, be.author, be.venue,
                    be.id
...
class CitationQueryHandler(QueryHandler):
    ...
    def convert_todf(self, query) -> pd.DataFrame: #auxiliary function
    ...
    def getAllCitations(self) -> pd.DataFrame:
        ...
        SELECT DISTINCT ?oci ?creation ?citing ?cited ?timespan ?journal_sc ?author_sc
        WHERE {{ 
            ?s cito:hasCreationDate ?creation .
            ?s rdfs:label ?oci .
            ?s cito:hasCitingEntity ?citing . 
    ...
    def getAllJournalSelfCitations(self) -> pd.DataFrame:
        query = """ PREFIX cito:  <http://purl.org/spar/cito/>
            ...
            SELECT DISTINCT ?oci ?creation ?citing ?cited ?timespan ?author_sc ?journal_sc
            WHERE {{ 
                ?s cito:hasCreationDate ?creation .
                ?s rdfs:label ?oci .
                ?s cito:hasCitingEntity ?citing . 
...
class BasicQueryEngine():
    ...
    def setFullDataFrame(self, be_df:pd.DataFrame, cit_df:pd.DataFrame) -> pd.DataFrame: #auxiliary function
    ...
    def constructBibliographicEntity(self, df) -> BibliographicEntity: #auxiliary function
    ...
    def constructCitation(self, class_to_construct = Citation) -> Citation: #auxiliary function
        result = list()
        beq = self.bibliographicEntityQuery

        for idx, row in class_to_construct.iterrows():
            CitedEntity = BibliographicEntity()
            CitingEntity = BibliographicEntity()
            if len(beq) > 0:
                for item in beq:
                    citing_df = item.getById(row["citing"][42:]) 
                    CitingEntity = self.constructBibliographicEntity(citing_df)[0]
                    
                    cited_df = item.getById(row["cited"][42:])
                    CitedEntity = self.constructBibliographicEntity(cited_df)[0]
            
            else:
                CitedEntity.id = row["cited"]
                CitingEntity.id = row["citing"]
            cit = Citation()
            cit.id = row["oci"]
            cit.creation = row["creation"]
            cit.timespan = row["timespan"]
            cit.hasCitedEntity = CitedEntity
            cit.hasCitingEntity = CitingEntity
            if row["author_sc"] == "yes" :
                isinstance(cit, AuthorSelfCitation)

            if row["journal_sc"] == "yes":
                isinstance(cit, JournalSelfCitation)  
            result.append(cit)        
        return result
    ...
    def getEntityById(self, id:str):
        def getCitationById(self, id):
            ...
            if ci_qhandler:
                for item in ci_qhandler:
                    i = item.getById(id)
                    if len(i) > 0:
                        return self.constructCitation(i)
            return None
        
        def getBibenById(self, id):
        ...
        result = None
        result = getBibenById(self, id)
        result = getCitationById(self, id)
        return result[0]
    ...
    def getAllCitations(self) -> list:
        ...
        for item in ci_qhandler:
            merge_ci = pd.concat([df_ci, item.getAllCitations()]) #selects the all of the Citation elements using a CQH method
    ...
    def getCitationsWithinTimespan(self, min_timespan:str, max_timespan:str) -> list:
        ...
        df_ci = pd.DataFrame(columns=["oci", "creation", "citing", "cited", "timespan"])
        for item in ci_qhandler:
            merge_ci = pd.concat([df_ci, item.getCitationsWithinTimespan(min_timespan, max_timespan)]) #selects the interested elements using a CQH method
    ...
    def getCitationsWithinDate(self, start_date:str, end_date:str) -> list:
        ...
        df_ci = pd.DataFrame(columns=["oci", "creation", "citing", "cited", "timespan"])
        for item in ci_qhandler:
            merge_ci = pd.concat([df_ci, item.getCitationsWithinDate(start_date, end_date)]) #selects the interested elements using a CQH method
    ...
    def constructBibliographicEntityList(self, df:pd.DataFrame) -> list: #da cancellare
        #additional function made to avoid repetitions in the code
    ...
    def constructCitationList(self, df:pd.DataFrame) -> list: #da cancellare
        list_of_ci = list()
        for idx, row in df.iterrows():
...
class FullQueryEngine(BasicQueryEngine):
    ...
    def getAuthorSelfCitationsByName(self, author_name:str) -> list:
        ...
        df_be = pd.DataFrame(columns=["internalId", "title", "author", "pub_date", "venue", "id"])
        for item in be_qhandler:
            merge_be = pd.concat([df_be, item.getAllBibliographicEntitiesWithAuthor(author_name)]) #selects only the BibliographicEntities with the desired author

        ci_qhandler = self.citationQuery
        df_ci = pd.DataFrame(columns=["oci", "creation", "citing", "cited", "timespan"])
        for item in ci_qhandler:
            merge_asc = pd.concat([df_ci, item.getAllAuthorSelfCitations()]) #selects only AuthorSelfCitations
    ...
    def getAuthorSelfCitationsByNameOld(self, author_name:str) -> list:
        result = list()
        asc_list = self.getAllAuthorSelfCitations()
        for asc in asc_list:
            if author_name in asc.getCitingEntity().getAuthors() and author_name in asc.getCitedEntity().getAuthors():
                result.append(asc)
        return result
    ...
    def getJournalSelfCitationsByName(self, journal_name:str) -> list:
        ...
        df_be = pd.DataFrame(columns=["internalId", "title", "author", "pub_date", "venue", "id"])
        for item in be_qhandler:
            merge_be = pd.concat([df_be, item.getAllBibliographicEntitiesWithVenue(journal_name)]) #selects only the BibliographicEntities with the desired venue

        ci_qhandler = self.citationQuery
        df_ci = pd.DataFrame(columns=["oci", "creation", "citing", "cited", "timespan"])
        for item in ci_qhandler:
            merge_jsc = pd.concat([df_ci, item.getAllJournalSelfCitations()]) #selects only JournalSelfCitations
    ...
    def getJournalSelfCitationsByNameOld(self, journal_name:str) -> list:
        result = list()
        jsc_list = self.getAllJournalSelfCitations() #gets a list of all JSC classes present in the dataframe
        for jsc in jsc_list:
            if jsc.getCitingEntity().getVenue() == journal_name and jsc.getCitedEntity().getVenue() == journal_name: #checks if the venues of getCitingEntity and getCitedEntity are the same
                result.append(jsc)
        return result
    ...
    def getCitationsOfBibEntityByTitleWithinDate(self, bib_entity_title:str, min_date:str, max_date:str) -> list:
        ...
        for idx, row in new_df.iterrows():
            ...
            ci.hasCitedEntity = self.constructBibliographicEntityList(row_be)[0]
            result.append(ci)  
        return result
